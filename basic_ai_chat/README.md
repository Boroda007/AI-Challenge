...
- **Единый чат**: одно сообщение пользователя и один ответ LLM с общей историей диалога
- **Системный промпт**: задаётся в нижней панели и добавляется в историю отдельным системным сообщением
- **Панель параметров**: Формат, Длина (токены), Стоп-символ, Температура, Reasoning
- **Бейджи у ответа**: применённые ограничения и причина остановки генерации
- **Счётчик токенов**: под ответом ИИ
- **Сырые данные**: раскрывающийся JSON-блок с запросом и ответом OpenAI API
...
├── routers/             # HTTP-маршруты: chat, pages, providers
├── services/            # Логика вызовов LLM и хранение истории
├── state.py             # Активный провайдер, модель и OpenAI-клиент
├── reasoning.py         # Capabilities моделей из models.json
├── models.json          # Реестр поддерживаемых reasoning capabilities
├── providers.json       # Конфигурация провайдеров и моделей (скрыт от Git)
...
### `POST /api/chat`
Принимает одно сообщение и ограничения генерации. Сервер использует собственную историю диалога, выполняет один вызов LLM и возвращает один ответ. История сохраняется только в памяти процесса и очищается при перезапуске сервера.
...
Добавляет системный промпт в конец серверной истории диалога. Пустое содержимое отклоняется с HTTP 400.
...
> **Примечания:**
> - Системный промпт добавляется в серверную историю через `POST /api/system-prompt` и не входит в `constraints`.
