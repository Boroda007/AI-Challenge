...
- **Режим агента (экспериментальный)**: агент с инструментами (`compare_responses`, `get_model_info`, `ask_user`), отображение шагов как диалога, граф сравнения вариантов
...
└── templates/
    ...
    ├── app.js           # Логика клиента (чат)
    └── agent.js         # Логика клиента (агент)
```
...
### `POST /api/agent` *(экспериментальный)*
Агентский режим: принимает `{ "message": "...", "history": [...] }`, возвращает ответ агента с шагами и результатами инструментов.
