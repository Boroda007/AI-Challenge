...
| `reasoning_effort` | Уровень рассуждений (`"none"`, `"low"`, `"medium"`, `"high"`) — только если поддерживается моделью |

> **Примечания:**
> - Системный промпт устанавливается отдельно через `POST /api/system-prompt` и не входит в `constraints`.
> - `reasoning_effort` передаётся в API только если задан в `constraints`.
